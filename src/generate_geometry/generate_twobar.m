...
function [X,T,row,col,solids,voids,F,freedofs] = generate_twobar(sizex,sizey,helem,doplot)
%% Define grid parameters 
dx = helem;             % element size in x-direction
dy = helem;             % element size in y-direction
l_load = dx*4;          % distribution length for filter boundary 
...
for e = 1:nelem
    ...
    if (x_cent>sizex-helem && abs(y_cent-sizey/2)<l_load*4/2); T(e,11) = 0; end % right face, near load
...
if (1<0)
    ...
    [supnodes2,~] = find(X(:,2)==sizey/20); % Find nodes with y=sizey/20
    supnodes = intersect(supnodes1,supnodes2);
    supdofs1 = [2*supnodes-1,2*supnodes];
    [supnodes1,~] = find(X(:,1)==0); % Find nodes with x=0
    [supnodes2,~] = find(X(:,2)==sizey-sizey/20); % Find nodes with y=sizey-sizey/20
    supnodes = intersect(supnodes1,supnodes2);
    supdofs2 = [2*supnodes-1,2*supnodes];
